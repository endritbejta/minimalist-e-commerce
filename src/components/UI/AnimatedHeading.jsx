...
const AnimatedHeading = ({
  ...
  const [shouldAnimate, setShouldAnimate] = useState(() => !hasRevealed(text));
  const [renderedText, setRenderedText] = useState(text);

  if (renderedText !== text) {
    setRenderedText(text);
    setShouldAnimate(!hasRevealed(text));
  }

  useEffect(() => {
    if (isActuallyVisible) markRevealed(text);
  }, [isActuallyVisible, text]);
